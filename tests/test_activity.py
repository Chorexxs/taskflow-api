...
def test_task_creation_logs_activity(client: TestClient):
    ...
    response = client.get(
        "/teams/test-team/projects/Test Project/tasks/Test Task/activity",
...
def test_task_status_change_logs_activity(client: TestClient):
    ...
    client.patch(
        "/teams/test-team/projects/Test Project/tasks/Test Task",
...
def test_task_assignment_logs_activity(client: TestClient):
    ...
    client.patch(
        "/teams/test-team/projects/Test Project/tasks/Test Task/assign",
...
def test_project_creation_logs_activity(client: TestClient):
    client.post("/auth/register", json={"email": "projlog@example.com", "password": "password123"})
    token = client.post("/auth/login", data={"username": "projlog@example.com", "password": "password123"}).json()["access_token"]
    
    client.post("/teams/", json={"name": "Log Team", "slug": "log-team"}, headers={"Authorization": f"Bearer {token}"})
    client.post("/teams/log-team/projects/", json={"name": "Log Project"}, headers={"Authorization": f"Bearer {token}"})
    
    response = client.get(
        "/teams/log-team/projects/Log Project/activity",
...
def test_role_change_logs_activity(client: TestClient):
    client.post("/auth/register", json={"email": "roleadmin@example.com", "password": "password123"})
    owner_token = setup_full_hierarchy_with_task(client, "rolelog@example.com")
    
    member_token = client.post("/auth/login", data={"username": "roleadmin@example.com", "password": "password123"}).json()["access_token"]
    client.post("/teams/test-team/members", json={"email": "roleadmin@example.com", "role": "member"}, headers={"Authorization": f"Bearer {owner_token}"})
    
    member_response = client.get("/teams/test-team/members", headers={"Authorization": f"Bearer {owner_token}"})
    member_id = member_response.json()[1]["user_id"]

    client.patch(
        f"/teams/test-team/members/{member_id}",
        json={"role": "admin"},
        headers={"Authorization": f"Bearer {owner_token}"}
    )
