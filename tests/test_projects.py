...
def setup_team(client: TestClient, email: str, password: str = "password123"):
    ...
    client.post("/api/v1/teams/", json={"name": "Test Team", "slug": "test-team"}, headers={"Authorization": f"Bearer {token}"})
    return token
...
def test_create_project(client: TestClient):
    ...
    response = client.post(
        "/api/v1/teams/test-team/projects/",
...
def test_get_project(client: TestClient):
    ...
    response = client.get(
        "/api/v1/teams/test-team/projects/Get Project",
        headers={"Authorization": f"Bearer {token}"}
    )
    assert response.status_code == 200
...
def test_update_project_as_admin(client: TestClient):
    ...
    response = client.patch(
        "/api/v1/teams/test-team/projects/Original Name",
...
def test_archive_project_as_admin(client: TestClient):
    ...
    response = client.delete(
        "/api/v1/teams/test-team/projects/To Archive",
        headers={"Authorization": f"Bearer {token}"}
    )
...
def test_member_cannot_archive_project(client: TestClient):
    ...
    response = client.delete(
        "/api/v1/teams/test-team/projects/Protected Project",
