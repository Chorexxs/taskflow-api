...
def setup_full_hierarchy_with_tasks(client: TestClient, owner_email: str):
    ...
    client.post("/teams/test-team/projects/Test Project/tasks/", json={"title": "Task 1", "priority": "high"}, headers={"Authorization": f"Bearer {owner_token}"})
    client.post("/teams/test-team/projects/Test Project/tasks/", json={"title": "Task 2", "priority": "medium"}, headers={"Authorization": f"Bearer {owner_token}"})
    client.post("/teams/test-team/projects/Test Project/tasks/", json={"title": "Task 3", "priority": "low"}, headers={"Authorization": f"Bearer {owner_token}"})
...
def test_filter_tasks_by_priority(client: TestClient):
    ...
    response = client.get(
        "/teams/test-team/projects/Test Project/tasks/?priority=high",
...
def test_pagination(client: TestClient):
    ...
    response = client.get(
        "/teams/test-team/projects/Test Project/tasks/?page=1&page_size=2",
        headers={"Authorization": f"Bearer {owner_token}"}
    )
    assert response.status_code == 200
    data = response.json()
...
def test_sort_tasks(client: TestClient):
    ...
    response = client.get(
        "/teams/test-team/projects/Test Project/tasks/?sort_by=title&order=asc",
        headers={"Authorization": f"Bearer {owner_token}"}
    )
    assert response.status_code == 200
    data = response.json()
...
def test_page_size_limit(client: TestClient):
    ...
    response = client.get(
        "/teams/test-team/projects/Test Project/tasks/?page_size=200",
        headers={"Authorization": f"Bearer {owner_token}"}
    )
    assert response.status_code == 200
    data = response.json()
